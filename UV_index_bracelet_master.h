...
float UVindex;
float lowUVthreshold = .01; // Neopixel light blinks green by default. Change thresholds as necessary.
float mediumUVthreshold = 7.1;
float highUVthreshold = 10.1;
// .01-7.0 Green light. Moderate to high risk of UV rays per World Health Organization.
// 7.1-10.0 Yellow light. Very high risk of UV rays per World Health Organization.
// 10.1-12 Red light. Extremely high risk of UV rays per World Health Organization.
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("Adafruit SI1145 test"); // Registers when UV index sensor comes up with an index reading
  if (! uv.begin()) {
